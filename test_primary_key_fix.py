...
def test_new_database():
    ...
    try:
        ...
        if db.index_exists('acct_fitid_posted'):
            print('✅ New unique index exists')
        else:
            print('❌ New unique index missing')
            
        if not db.index_exists('acct_fitid'):
            print('✅ Old index not present')
        else:
            print('❌ Old index still exists')
...
def test_migration():
    ...
    try:
        ...
        if db.index_exists('acct_fitid_posted'):
            print('✅ New unique index exists after migration')
        else:
            print('❌ New unique index missing after migration')
            
        if not db.index_exists('acct_fitid'):
            print('✅ Old index removed after migration')
        else:
            print('❌ Old index still exists after migration')
        
        # Test that records still exist
        records = db.all_records()
        print(f'Records in database: {len(records)}')
        if len(records) == 2:
            print('✅ All records preserved during migration')
        else:
...
def test_duplicate_fitid_import():
    ...
    try:
        ...
        if len(records) == 2:
            print('✅ Both records imported successfully!')
            print('✅ Primary key collision fix working!')
        ...
        if specific_record:
            ...
            print(f'✅ Found specific record: {rec["name"]} on {rec["posted"][:10]}')
        else:
            print('❌ Could not find specific record')
...
def test_category_assignment():
    ...
    try:
        ...
        if coffee_record['category'] != gas_record['category']:
            print('✅ Different categories assigned to each record')
            print('✅ Precise category assignment working!')
        else:
            print('❌ Same category assigned to both records')
        ...
        try:
            ...
            print('❌ Should have failed with non-existent fitid')
        except Exception as e:
            print(f'✅ Correctly rejected invalid fitid: {str(e)}')
